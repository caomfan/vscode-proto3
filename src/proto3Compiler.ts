...
export class Proto3Compiler {
  ...
  public compileActiveProtoWithoutProtoPath() {
    let editor = vscode.window.activeTextEditor;
    if (editor && editor.document.languageId == "proto3") {
      let fileName = editor.document.fileName;
      let dirName = path.dirname(fileName);
      let args = this._config
        .getProtocOptions()
        .filter((item) => !item.startsWith("--proto_path") && !item.startsWith("--cpp_out"));
      args = args.concat("--proto_path=" + dirName, "--cpp_out=" + dirName, path.basename(fileName));
